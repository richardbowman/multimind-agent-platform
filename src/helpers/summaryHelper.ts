...
class SummaryHelper {
    async summarizeContent(task: string, content: string, llmService: ILLMService): Promise<string> {
        ...
        const summary = await llmService.sendMessageToLLM(userPrompt, history);

        return summary;
    }

    async createOverallSummary(goal: string, task: string, summaries: string[], lmStudioService: ILLMService): Promise<string> {
        ...
        return await llmService.sendMessageToLLM(userPrompt, history);
    }
}
