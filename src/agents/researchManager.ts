...
import { WEB_RESEARCH_CHANNEL_ID, PROJECTS_CHANNEL_ID, CHROMA_COLLECTION, ORCHESTRATOR_USER_ID, RESEARCHER_USER_ID } from '../helpers/config';
...
export class ResearchManager extends Agent<ResearchProject, ResearchTask> {
    ...
    protected async projectCompleted(project: ResearchProject): Promise<void> {
        ...
        const overallSummary = await this.createFinalReport(project, aggregatedData);
        ...
        await this.chatClient.postReply(post.id, PROJECTS_CHANNEL_ID, overallSummary);
        ...
        Logger.info(`Report saved as artifact with ID ${project.id}`);
    }
    ...
    `;
            ...
            if (responseJSON.researchRequested) {
                for (const task of responseJSON.researchRequested) {
                    ...
                    this.projects.addTask(project, new ResearchTask(taskId, projectId, task, ORCHESTRATOR_USER_ID));
                }
            } else {
...
${tasks.map(({ description }) => ` - ${description}`).join("\n")}`;
    ...
    private async createFinalReport(project: ResearchProject, aggregatedData: string): Promise<string> {
        try {
            const systemPrompt = `
